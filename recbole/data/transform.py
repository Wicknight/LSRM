...
def construct_transform(config):
    ...
    if config["transform"] is None:
        ...
        str2transform = {
            ...
            "noise_seq": NoisySequence,
            "sparse_seq": SparseSequence,
        }
...
class NoisySequence:
    ...
    def __init__(self, config):
        self.ITEM_ID = config["ITEM_ID_FIELD"]
        self.ITEM_SEQ = config["ITEM_ID_FIELD"] + config["LIST_SUFFIX"]
        self.NOISY_ITEM_SEQ = "Noisy_" + self.ITEM_SEQ
        self.ITEM_SEQ_LEN = config["ITEM_LIST_LENGTH_FIELD"]
        self.NOISY_ITEM_SEQ_LEN = self.NOISY_ITEM_SEQ + self.ITEM_SEQ_LEN
        self.noise_eta = config["robustness_eta"]
        config["NOISY_ITEM_SEQ"] = self.NOISY_ITEM_SEQ
    ...
    def __call__(self, dataset, interaction):
        ...
        for seq, length in zip(item_seq, item_seq_len):
            noisy_item_seq = seq.cpu().detach().numpy().copy()
            for index_id, item in enumerate(seq):
                if item == 0: 
                    break
                prob = random.random()
                if prob < self.noise_eta:
                    if prob < self.noise_eta / 2:
                        noisy_item = random.randint(1, n_items-1)
                    else:
                        noisy_item = 0
                    noisy_item_seq[index_id] = noisy_item
            noisy_item_seq_list.append(
                torch.tensor(noisy_item_seq, dtype=torch.long, device=device)
            )
        new_dict = {
            self.NOISY_ITEM_SEQ: torch.stack(noisy_item_seq_list),
        }
        interaction.update(Interaction(new_dict))
        return interaction
...
class SparseSequence:
    ...
    def __init__(self, config):
        self.ITEM_ID = config["ITEM_ID_FIELD"]
        self.ITEM_SEQ = config["ITEM_ID_FIELD"] + config["LIST_SUFFIX"]
        self.SPARSE_ITEM_SEQ = "Sparse_" + self.ITEM_SEQ
        self.ITEM_SEQ_LEN = config["ITEM_LIST_LENGTH_FIELD"]
        self.SPARSE_ITEM_SEQ_LEN = self.SPARSE_ITEM_SEQ + self.ITEM_SEQ_LEN
        self.sparse_eta = config["sparse_eta"]
        config["SPARSE_ITEM_SEQ"] = self.SPARSE_ITEM_SEQ
        config["SPARSE_ITEM_SEQ_LEN"] = self.SPARSE_ITEM_SEQ_LEN
    ...
    def __call__(self, dataset, interaction):
        ...
        item_seq_len = interaction[self.ITEM_SEQ_LEN]
        device = item_seq.device
        sparse_item_seq_list, sparse_item_seqlen_list = [], []

        for seq, length in zip(item_seq, item_seq_len):
            sparse_len = math.ceil(length*self.sparse_eta)
            sparse_item_seq = []
            for index_id, item in enumerate(seq):
                if item == 0:
                    break
                if index_id >= length-sparse_len: 
                    sparse_item_seq.append(item)
            
            sparse_item_seqlen_list.append(
                torch.tensor(sparse_len, dtype=torch.long, device=device)
            )

            while(len(sparse_item_seq) < len(seq)):
                sparse_item_seq.append(0)

            sparse_item_seq_list.append(
                torch.tensor(sparse_item_seq, dtype=torch.long, device=device)
            )
        new_dict = {
            self.SPARSE_ITEM_SEQ: torch.stack(sparse_item_seq_list),
            self.SPARSE_ITEM_SEQ_LEN: torch.stack(sparse_item_seqlen_list),
        }
        interaction.update(Interaction(new_dict))
        return interaction
